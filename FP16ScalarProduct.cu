...
int main(int argc, char** argv, char** env)
{
    ...
    if(devProp.mahor<5 || (devProp.major == 5 && devProp.minor<3))
    {
    ...
    float results_native = 0;
    for(int i=0; i<NUM_OF_BLOCKS; ++i)
    {
        results_native += results[i];
    }
    printf("results native operators\t:%f\n", result_native);
